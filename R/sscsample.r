sscsample <- function (size, n.samples, sample.type = "simple", x = NULL, strata = NULL, cluster = NULL, ret = FALSE) 
{
  ...
  draw.stratum <- function(thresholds) {
    r <- runif(1)
    for (i in 1:length(thresholds)) if (r < thresholds[i]) {stratum <- i; break;}
    return(stratum)
  }
  ...
  if (sample.type == "stratified" | sample.type == 2) {
    idx.vec <- 1:nx
    stratified.data <- lapply(strata.names,group.idx.by.name,names.vec=strata,idx=idx.vec)
    ...
    if (sample.unit.residuals > 0) {    ## prepare the random draw of the residual units
      for (i in 2:length(sample.strata.fractions)) sample.strata.fractions[i] <- sample.strata.fractions[i] + sample.strata.fractions[i-1] 
  ...
  for (r in 1:n.samples) {
    if (sample.type == "simple" | sample.type == 1) {
      ...
      for (stratum in 1:n.strata) {     ## Sample the whole units from all strata
        ...
        else sample.idx <- c(sample.idx, sample(stratified.data[[stratum]], 
                                                sample.strata.units[stratum]))
      }
  ...
  for (r in 1:n.samples) {
    idx <- samples[, r]
    means[r] <- mean(x[idx])
    for (j in 1:n.strata) s.strata[r, j] <- sum(strata[idx] == 
                                                strata.names[j])
    sample.out[, r] <- x[idx]
    cat(paste(r, "\t", round(means[r], 4), "\t", s.strata[r, 
                                                          1], "\t\t", s.strata[r, 2], "\t\t", s.strata[r, 3], 
              "\n", sep = ""))
  }
  if (ret) return(list(samples = samples, s.strata = s.strata, means = means))
}
